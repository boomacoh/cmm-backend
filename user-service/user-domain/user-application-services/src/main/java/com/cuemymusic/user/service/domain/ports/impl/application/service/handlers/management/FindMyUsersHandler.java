...
@Component
@Slf4j
public class FindMyUsersHandler {
    private final UserManagerRepository userManagerRepository;
    private final UserRepository userRepository;
    private final UserManagementMapper userManagementMapper;
    private final AuthenticationService authenticationService;

    public FindMyUsersHandler(UserManagerRepository userManagerRepository,
    ...
    public FindUsersResponse find(String token, UUID club){
        ...
        List<User> users = this.userManagerRepository.findMyUsers(user.getId().getValue(), club).stream().filter(e->e.getRole() == Role.USER).toList();
